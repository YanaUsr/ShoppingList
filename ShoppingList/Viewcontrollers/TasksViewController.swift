...
//  ShoppingList
//
//  Created by Яна Иноземцева on 20.05.2022.
//
...
class TasksViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let tasks = taskList.tasks.self[indexPath.row]
        ...
        let doneAction = UIContextualAction(style: .normal, title: doneTitle) { _, _, isDone in
            ...
            let indexPathForCurrentTask = IndexPath(
                row: self.currentTasks.index(of: tasks) ?? 0,
                section: 0
            )
            let indexPathForCompletedTask = IndexPath(
                row: self.completedTasks.index(of: tasks) ?? 0,
                section: 1
            )
    ...
    private func showAlert(with task: Task? = nil, completion: (() -> Void)? = nil) {
        ...
        alert.action(with: task) { newValue, note in
            if let task = task, let completion = completion {
                // TODO - edit task
                StorageManager.shared.editNote(task, newName: newValue, newValueOfNote: note)
                completion()
            } else {
